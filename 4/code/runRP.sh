    ...
    case "$run" in
        $runnames)
            ...
            then
                ...
                if [[ -f "../work/$run/lock.rpc" ]]
                then
                    echo "Data currently reconstructing. Skipping Run $run."
                elif [[ -f "../work/$run/lock.icf" ]]
                then
                    echo "Data currently simulating. Skipping Run $run."
                elif [[ -f "../work/$run/log.icoFoam" ]]
                then
                    cd ../work/"$run"
                    echo $PWD
                    echo "-\ Staging"
                    ...
                    touch lock.rpc
                    echo "-\ RPrun"
                    { time ./RPrunParallel 2>> time.txt ; }
                    echo "-\ RP Complete."
                    echo "-\ Post-processing via Sample..."
                    # postProcess -cdfunc sample -latestTime
